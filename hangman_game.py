...
def input_let_word():
    letters_words = input('Введите букву или слово целиком: ').upper()
    while not letters_words.isalpha():
        letters_words = input('Ввести можно ТОЛЬКО букву или слово целиком: ').upper()
...
def play(word):
    ...
    while guessed:  # цикл угадивания слов
        print(display_hangman(tries))
        print(word_completion)
        print(f'Осталось попыток: {tries}')
        letters_words = input_let_word()
        while (letters_words in guessed_letters) or (letters_words in guessed_words):
            print('Такая буква или слово уже были!!!')
            letters_words = input_let_word()
        if len(letters_words) == 1:
            if letters_words in word:
                print('Да, такая буква в слове есть!')
                guessed_letters.append(letters_words)
                word_completion = ''
                for c in word:
            ...
            if letters_words == word:
                print('Поздравляю Вы победили!!!')
                guessed = False
            else:
                print('Неправильно!')
                tries -= 1
            guessed_words.append(letters_words)
        if tries == 0:
            guessed = False
    if tries == 0:
        display_hangman(tries)
        print('Вы не справились :(! Вас повесили.')
        print(f'Я загадал для тебя слово: {word}')


play(get_word())

while next_game:
    x = input('Хотите повторить? (да - 1, нет - enter): ')
    print()
    if x == '1':
        play(get_word())
    else:
